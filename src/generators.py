...
def configuration_model_from_directed_multigraph(graph, seed=None):
    ...
    in_deg_seq = [d[1] for d in graph.in_degree]
    out_deg_seq = [d[1] for d in graph.out_degree]

    return nx.directed_configuration_model(in_deg_seq, out_deg_seq, seed=seed)
